...
bytes from `GET /artifacts/<id>/download` or data URLs from `GET
...
project files. Android checks accept `ANDROID_HOME` or `ANDROID_SDK_ROOT` for
the SDK path, and `NDK_HOME` or `ANDROID_NDK_HOME` for the NDK path.
